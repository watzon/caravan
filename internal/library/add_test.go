...
func TestAddMovieCreatesTheRowWithoutTouchingDisk(t *testing.T) {
	...
	mv, err := h.mgr.AddMovie(ctx, 10378, "", nil, 0)
	if err != nil {
...
func TestAddMovieAgainKeepsUserIntent(t *testing.T) {
	...
	first, err := h.mgr.AddMovie(ctx, 10378, "", nil, 0)
	if err != nil {
		t.Fatalf("AddMovie: %v", err)
	}
	...
	second, err := h.mgr.AddMovie(ctx, 10378, "", nil, 0)
	if err != nil {
...
func TestAddSeriesWritesTheWholeTree(t *testing.T) {
	...
	sr, err := h.mgr.AddSeries(ctx, 66732, nil, 0)
	if err != nil {
...
func TestAddSeriesLeavesSpecialsUnmonitored(t *testing.T) {
	...
	sr, err := h.mgr.AddSeries(ctx, meta.TMDBID, nil, 0)
	if err != nil {
		t.Fatalf("AddSeries: %v", err)
	}
	...
	if _, err := h.mgr.AddSeries(ctx, meta.TMDBID, nil, 0); err != nil {
...
func TestAddWithoutProviderIsRecognizable(t *testing.T) {
	...
	if _, err := mgr.AddMovie(context.Background(), 1, "", nil, 0); !errors.Is(err, core.ErrNoMetadataProvider) {
		t.Fatalf("AddMovie error = %v, want core.ErrNoMetadataProvider", err)
	}
	if _, err := mgr.AddSeries(context.Background(), 1, nil, 0); !errors.Is(err, core.ErrNoMetadataProvider) {
...
func TestAddRejectsInvalidProviderID(t *testing.T) {
	...
	if _, err := h.mgr.AddMovie(context.Background(), 0, "", nil, 0); err == nil {
		t.Fatal("AddMovie(0) succeeded, want an error")
	}
	if _, err := h.mgr.AddSeries(context.Background(), -1, nil, 0); err == nil {
...
func TestAddMovieMinAvailability(t *testing.T) {
	...
	mv, err := h.mgr.AddMovie(ctx, 10378, core.AvailabilityAnnounced, nil, 0)
	if err != nil {
		t.Fatalf("AddMovie: %v", err)
	}
	...
	kept, err := h.mgr.AddMovie(ctx, 10378, "", nil, 0)
	if err != nil {
		t.Fatalf("AddMovie again: %v", err)
	}
	...
	changed, err := h.mgr.AddMovie(ctx, 10378, core.AvailabilityInCinemas, nil, 0)
	if err != nil {
...
func TestAddMovieHonoursTheMonitoredChoice(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			h := newHarness(t)
			h.provider.movieByID[10378] = core.MovieMeta{TMDBID: 10378, Title: "Big Buck Bunny", Year: 2008}
			mv, err := h.mgr.AddMovie(ctx, 10378, "", tt.monitored, 0)
			if err != nil {
				t.Fatalf("AddMovie: %v", err)
			}
...
func TestAddMovieLeavesAnExistingRowsMonitoredFlagAlone(t *testing.T) {
	...
	if _, err := h.mgr.AddMovie(ctx, 10378, "", ptr(true), 0); err != nil {
		t.Fatalf("AddMovie: %v", err)
	}
	again, err := h.mgr.AddMovie(ctx, 10378, "", ptr(false), 0)
	if err != nil {
		t.Fatalf("AddMovie again: %v", err)
	}
	if !again.Monitored {
...
func TestAddSeriesUnmonitoredLeavesTheWholeTreeUnmonitored(t *testing.T) {
	...
	sr, err := h.mgr.AddSeries(ctx, 66732, ptr(false), 0)
	if err != nil {
		t.Fatalf("AddSeries: %v", err)
	}
	if sr.Monitored {
