...
// filed under a 6000-series category, never because of how it looks. A date in
// a television name is a daily episode, which is explicitly out of scope (SPEC
// §16), and letting Parse guess would change what existing names mean. The
// second is what identity means here: a scene has no season/episode tag,
...
// source, codec, audio, bit depth, edition, PROPER/REPACK and the release group
// all come from the same rule tables, so an adult release and a television
// release describe their video the same way.
//
// A name that carries no usable date falls through to Parse. That is what makes
// the category selector safe: an indexer that files a television or movie
// release under XXX still yields a correct parse, and the caller tells the two
// apart with ParsedRelease.IsScene.
...
// date-SHAPED and are not dates, and 2021 has no February 29th: treating any of
// them as a date would file a scene under a season that does not exist, so each
// is skipped and the scan continues rather than being rolled forward the way
// time.Date would.
