...
// ---------------------------------------------------------------------------
// The dual-writes: the module switch and the per-account flag keep saying what
// they always said, and say it in the new columns too, until the access API
// replaces them.
// ---------------------------------------------------------------------------

func TestSetAdultEnabledMirrorsOntoLibraryActive(t *testing.T) {
	ctx := context.Background()
	st, _ := openTemp(t)
	...
	// A second adult library, alongside the one the enable created.
	second := &core.Library{Kind: core.LibraryKindAdult, Name: "Scenes",
		RootPath: "library/Scenes", Providers: []string{core.ProviderStashbox}}
	if err := st.CreateLibrary(ctx, second); err != nil {
		t.Fatalf("CreateLibrary: %v", err)
	}

	adultActive := func() []bool {
		t.Helper()
		libs, err := st.ListLibrariesByKind(ctx, core.LibraryKindAdult)
		if err != nil {
			t.Fatalf("ListLibrariesByKind(adult): %v", err)
		}
		out := make([]bool, 0, len(libs))
		for _, l := range libs {
	...
	if got := adultActive(); !reflect.DeepEqual(got, []bool{true, true}) {
		t.Errorf("adult libraries active = %v, want both on under an enabled module", got)
	}

	if err := st.SetAdultEnabled(ctx, false); err != nil {
		t.Fatalf("SetAdultEnabled(false): %v", err)
	}
	if got := adultActive(); !reflect.DeepEqual(got, []bool{false, false}) {
		t.Errorf("adult libraries active = %v, want both off under a disabled module", got)
	}
	// Off deletes nothing — both rows are still there to come back on.
	if err := st.SetAdultEnabled(ctx, true); err != nil {
		t.Fatalf("SetAdultEnabled(true) again: %v", err)
	}
	if got := adultActive(); !reflect.DeepEqual(got, []bool{true, true}) {
		t.Errorf("adult libraries active = %v after a re-enable, want both on", got)
	}

	// Nothing else on the install is touched by the module's switch.
	tv, err := st.GetLibraryByKind(ctx, core.LibraryKindTV)
	if err != nil {
		t.Fatalf("GetLibraryByKind(tv): %v", err)
	}
	if !tv.Active {
		t.Error("the module switch reached a television library")
	}
}

func TestSetUserAdultAccessMirrorsOntoLibraryAccess(t *testing.T) {
	ctx := context.Background()
	st, _ := openTemp(t)
	...
	if err := st.SetUserAdultAccess(ctx, alice.ID, true); err != nil {
	...
	if want := map[int64]bool{adult.ID: true}; !reflect.DeepEqual(grants, want) {
		t.Errorf("grants = %v, want %v — the mirror writes adult libraries only", grants, want)
	}
	// The grant is the module's, so it must not reach a television library.
	if grants[kids.ID] {
		t.Error("the adult grant landed on a television library")
	}

	// Granting twice writes one row rather than failing on the composite key.
	if err := st.SetUserAdultAccess(ctx, alice.ID, true); err != nil {
		t.Fatalf("SetUserAdultAccess(true) again: %v", err)
	}
	roster, err := st.ListLibraryAccess(ctx, adult.ID)
	if err != nil {
		t.Fatalf("ListLibraryAccess: %v", err)
	}
	if want := []int64{alice.ID}; !reflect.DeepEqual(roster, want) {
	...
	if err := st.SetUserAdultAccess(ctx, alice.ID, false); err != nil {
	...
	if len(roster) != 0 {
		t.Errorf("roster = %v after a revoke, want empty", roster)
	}
}
