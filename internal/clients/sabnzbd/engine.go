...
// ListPage returns a deterministic page over SABnzbd's current snapshot. The
// provider has no replay cursor, so callers must retain the legacy fallback
// when this optional seam is unavailable or incomplete.
